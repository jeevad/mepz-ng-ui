...
export class TacComponent implements OnInit {
  ...
  filterPackage = (item: any): boolean => {
    return item.package
      ?.toLowerCase()
      .includes(this.filterValues[0].toLowerCase());
  };

  filterGroup = (item: any): boolean => {
    return item.group
      ?.toLowerCase()
      .includes(this.filterValues[1].toLowerCase());
  };

  filterEqCode = (item: any): boolean => {
    return item.code
      ?.toLowerCase()
      .includes(this.filterValues[2].toLowerCase());
  };

  filterEquipment = (item: any): boolean => {
    return item.name
      ?.toLowerCase()
      .includes(this.filterValues[3].toLowerCase());
  };

  filterBrand = (item: any): boolean => {
    return item.brands
      ?.toLowerCase()
      .includes(this.filterValues[4].toLowerCase());
  };

  filterModel = (item: any): boolean => {
    return item.model
      ?.toLowerCase()
      .includes(this.filterValues[5].toLowerCase());
  };

  filterCountry = (item: any): boolean => {
    return item.country_of_origin
      ?.toLowerCase()
      .includes(this.filterValues[6].toLowerCase());
  };

  filterSupplier = (item: any): boolean => {
    return item.supplier_firm
      ?.toLowerCase()
      .includes(this.filterValues[7].toLowerCase());
  };

  filterActDelivery = (item: any): boolean => {
    return item.delivery_actual
      ?.toLowerCase()
      .includes(this.filterValues[8].toLowerCase());
  };

  filterEstDelivery = (item: any): boolean => {
    return item.est_delivery
      ?.toLowerCase()
      .includes(this.filterValues[9].toLowerCase());
  };

  onFilterChange(index: number, event: any) {
    this.filterValues[index] = event.target.value;
    this.filterFunctions[index] =
      index === 0
        ? this.filterPackage
        : index === 1
        ? this.filterGroup
        ...
        ? this.filterSupplier
        : index === 8
        ? this.filterActDelivery
        : this.filterEstDelivery;
    this.applyFilters();
  }
